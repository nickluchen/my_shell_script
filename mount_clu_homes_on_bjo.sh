...
then
    ...
    then
        cmd='sudo mount'
        check_and_mount "${cmd}" "-t cifs" //${bjo_file_server}/users/clu ${clu_domain_home} "-o user=clu,domain=DOLBYNET,uid=clu,gid=clu,$PASSWD_OPTION"
        check_and_mount "${cmd}" "-t cifs" //${bjo_bld_server}/clu ${clu_bld_home} "-o user=clu,domain=DOLBYNET,uid=clu,gid=clu,$PASSWD_OPTION"
    ...
    then
        cmd=mount
        check_and_mount "${cmd}" "-t smbfs" //clu@${bjo_bld_server}/clu        ${clu_bld_home}
        check_and_mount "${cmd}" "-t smbfs" //clu@${bjo_file_server}/users/clu ${clu_domain_home}
